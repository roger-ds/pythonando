...
def solicitar_exames(request):
    ...
    if request.method == "GET":
        return render(
            request, "solicitar_exames.html", {"tipos_exames": tipos_exames}
        )
...
def gerenciar_pedidos(request):
    ...
    return render(
        request, "gerenciar_pedidos.html", {"pedidos_exames": pedidos_exames}
    )
...
def cancelar_pedido(request, pedido_id):
    ...
    if not pedido.usuario == request.user:
        messages.add_message(
            request,
            constants.ERROR,
            "Esse pedido não é seu, Portanto, você não pode cancelar esse pedido",
        )
        return redirect("/exames/gerenciar_pedidos")
    ...
    pedido.save()
    messages.add_message(
        request, constants.SUCCESS, "Pedido cancelado com sucesso"
    )
    return redirect("/exames/gerenciar_pedidos")
